...
import * from 'jquery'
let handleJsonData = function(zip, filename) {
  let $pass = $("#pass");
  let $meta = $("#meta dl");
  $meta.html("");
  ...
  zip
    ...
    .then(
      function success(content) {
        let passData = JSON.parse(content);
        ...
        if (passData.eventTicket) {
          if (passData.eventTicket.headerFields) {
            ...
            jQuery.each(passData.eventTicket.headerFields, function(
              key,
              field
            ) {
          ...
          if (passData.eventTicket.primaryFields) {
            ...
            jQuery.each(passData.eventTicket.primaryFields, function(
              key,
              field
            ) {
              $fields.append("<dt>" + field.label + "</dt>");
          ...
          if (passData.eventTicket.secondaryFields) {
            ...
            jQuery.each(passData.eventTicket.secondaryFields, function(
              key,
              field
            ) {
              $fields.append("<dt>" + field.label + "</dt>");
          ...
          if (passData.eventTicket.auxiliaryFields) {
            ...
            jQuery.each(passData.eventTicket.auxiliaryFields, function(
              key,
              field
            ) {
              $fields.append("<dt>" + field.label + "</dt>");
        ...
        $meta.append(
          "<dt>Description</dt><dd>" + passData.description + "</dd>"
        );
        $meta.append(
          "<dt>Organization Name</dt><dd>" + passData.organizationName + "</dd>"
        );
        $meta.append(
          "<dt>Pass Type Identifier</dt><dd>" +
            passData.passTypeIdentifier +
            "</dd>"
        );
        $meta.append(
          "<dt>Serial Number</dt><dd>" + passData.serialNumber + "</dd>"
        );
        $meta.append(
          "<dt>Team Identifier</dt><dd>" + passData.teamIdentifier + "</dd>"
        );
        $meta.append(
          "<dt>Format Version</dt><dd>" + passData.formatVersion + "</dd>"
        );
      },
      function error(e) {
        // handle the error
...
let handleForegroundImage = function(zip, filename, $element) {
  zip
    .file(filename)
    .async("base64")
    .then(
...
let handleBackgroundImage = function(zip, filename, $element) {
  zip
    .file(filename)
    .async("base64")
    .then(
